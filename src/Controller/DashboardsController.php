...
class DashboardsController extends AppController
{
	...
	public function index()
	{
		...
		$total_student = $students->find()->all()->count();
		$active_student = $students->find()->where(['status' => 1])->count();
		$student_percent = $active_student * 100 / $total_student;

		$houses = $this->fetchTable('Houses');
		$total_house = $houses->find()->all()->count();
		$verified_house = $houses->find()->where(['status' => 2])->count();
		$house_percent = $verified_house * 100 / $total_house;
		
		$reservations = $this->fetchTable('Reservations');
		$total_reservation = $reservations->find()->all()->count();
		$approved_reservation = $reservations->find()->where(['status' => 0])->count();
		$reservation_percent = $approved_reservation * 100 / $total_reservation;
		...
		$this->set(compact('total_user','total_student', 'total_house','total_reservation', 'total_contact', 'total_auditlog', 'total_todo', 'user_percent', 'pending_todo_percent', 'pending_faq_percent', 'pending_contact_percent', 'userLogs', 'formattedResults', 'totalActivityByMonth', 'todo_list'));
	}
}
