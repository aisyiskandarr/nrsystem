    ...
    public function index()
{
    ...
    $user = $this->Authentication->getIdentity(); // get logged-in user
    $hasStudent = $this->Students->exists(['users_id' => $user->id]);
    $this->set('hasStudent', $hasStudent);
    $query = $this->Students->find('search', search: $this->request->getQueryParams())
        ->contain(['Users'])
        ->where(['users_id' => $user->id]); // restrict to own record only
