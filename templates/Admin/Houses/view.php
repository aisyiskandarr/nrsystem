...
<div class="text-center mt-4">
    ...
    <div class="col-md-3">
	  <div class="card bg-body-tertiary border-0 shadow">
        <div class="card-body">
            <div class="card-title mb-0">Houses Verification</div>
            <div class="tricolor_line mb-3"></div>
            <table class ="table table-sm table-hover">
                <tr>
                    <td width="35%">Date</td>

                    <td><?php echo date('M d, Y (h:i A)', strtotime($house->created)); ?></td>
                </tr>
                <tr>
                    <td>House Status</td>
                    <td width="15%"class="text-end"><?php if ($house -> status == 0) {
                                echo '<span class= "badge bg-danger">Blaclist</span>';
                            } elseif ($house -> status == 1) {
                                echo '<span class= "badge bg-warning">Unverified</span>';
                            } elseif ($house -> status == 2) {
                                echo '<span class= "badge bg-success">Verified</span>';
            ...
            <?php if ($house->status == 0) : // Approved ?>
    <div class="mb-3 col-12 mb-0">
        <div class="alert alert-warning">
            <div class="fw-semibold">Unverified House</div>
            <p class="mb-0">Are you sure you want to mark <?= h($house->owner) ?> house as unverified?</p>
            <div class="text-end mb-2">
                <?= $this->Form->postLink(
                    __('Mark as Pending'),
                    ['action' => 'changeStatus', $house->id, 1],
                    [
                        'confirm' => __('Are you sure you want to mark user: "{0}" as unverified?', $house->owner),
                        'title' => __('Unverified'),
...
<?php elseif ($house->status == 1) : // Pending ?>
    <div class="mb-3 col-12 mb-0">
        <div class="alert alert-info">
            <div class="fw-semibold">Verified House</div>
            <p class="mb-0">Are you sure you want to verify <?= h($house->owner) ?> house?</p>
            <div class="text-end">
                <?= $this->Form->postLink(
                    __('Approve'),
                    ['action' => 'changeStatus', $house->id, 2],
                    [
                        'confirm' => __('Are you sure you want to approve user: "{0}"?', $house->owner),
                        'title' => __('Verify'),
    ...
    <div class="mb-3 col-12 mb-0">
        <div class="alert alert-danger">
            <div class="fw-semibold">Blaclist House</div>
            <p class="mb-0">Are you sure you want to blacklist <?= h($house->owner) ?> house?</p>
            <div class="text-end">
                <?= $this->Form->postLink(
                    __('Reject'),
                    ['action' => 'changeStatus', $house->id, 0],
                    [
                        'confirm' => __('Are you sure you want to blacklist user: "{0}"?', $house->owner),
                        'title' => __('Blacklist'),
...
<?php elseif ($house->status == 2) :  ?>
    <div class="mb-3 col-12 mb-0">
        <div class="alert alert-warning">
            <div class="fw-semibold">Unverify Booking Booking</div>
            <p class="mb-0">Are you sure you want to mark <?= h($house->owner) ?> house as unverified?</p>
            <div class="text-end">
                <?= $this->Form->postLink(
                    __('Mark as unverified'),
                    ['action' => 'changeStatus', $house->id, 1],
                    [
                        'confirm' => __('Are you sure you want to mark user: "{0}" as unverified?', $house->owner),
                        'title' => __('Mark as Unverified'),
                        'class' => 'btn btn-warning btn-xs',
                        'escapeTitle' => false,
                        'data-bs-toggle' => "modal",
                        'data-bs-target' => "#bootstrapModal"
