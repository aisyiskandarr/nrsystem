		...
		?>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?=($reservation->student->matrix_number)?></td>
					<td><?= ($reservation->house->address)?></td>
                    <td><?php echo date('d F Y', strtotime($reservation->reservation_date));?></td>
                    <td><?= h($reservation->contact) ?></td>
                    <td><?= h($reservation->nric) ?></td>
                    
					<td class="text-center"><?php if ($reservation->studentcard == NULL) {
							echo '<i class="fa-solid fa-circle-xmark text-danger"></i>';
						}elseif  ($reservation->studentcard != NULL) {
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $reservation->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_reservations_active; ?></h3>
					<p>Active Reservations</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_reservations_archived; ?></h3>
					<p>Archived Reservations</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Reservations (Monthly)</div>
			<div class="gradient_line mb-3"></div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Reservations by Status</div>
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Reservations(s)',
			data: [<?= json_encode($total_reservations_active); ?>, <?= json_encode($total_reservations_disabled); ?>, <?= json_encode($total_reservations_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="gradient_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Reservations','action' => 'index']]); ?>
				<fieldset>
					<div class="mb-1"><?php echo $this->Form->control('id',['required' => false]); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('students_id', ['options' => $students, 'empty' => 'Select Students', 'class' => 'form-select']); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('houses_id', ['options' => $houses, 'empty' => 'Select Owner', 'class' => 'form-select']); ?></div>
