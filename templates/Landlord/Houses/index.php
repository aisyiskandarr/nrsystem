 ...
 * @var iterable<\App\Model\Entity\House> $houses
 */
		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New House'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('housing_area') ?></th>
                    <th><?= $this->Paginator->sort('address') ?></th>
                    <th><?= $this->Paginator->sort('capacity') ?></th>
                    <th><?= $this->Paginator->sort('amount_person') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    <td><?= h($house->owner) ?></td>
                    <td><?= h($house->housing_area) ?></td>
                    <td><?= h($house->address) ?></td>
                    <td class="text-center"><?= $this->Number->format($house->capacity) ?></td>
                    <td><?= h($house->amount_person) ?></td>
					<td class="text-center"><?php if ($house->image == NULL) {
							echo '<i class="fa-solid fa-circle-xmark text-danger"></i>';
						}elseif  ($house->image != NULL) {
						echo '<i class="fa-solid fa-circle-check text-success"></i>';
						}
						?></td>
                    <td><?= h($house->contact) ?></td>
                    <td class="text-center"><?php if ($house -> availability == "Available") {
					...
					<td class="actions text-center"><?php if ($house -> status == 0) {
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $house->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $house->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Houses: "{0}"?', $house->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_houses_active; ?></h3>
					<p>Active Houses</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_houses_archived; ?></h3>
					<p>Archived Houses</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Houses (Monthly)</div>
			<div class="gradient_line mb-3"></div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Houses by Status</div>
			<div class="gradient_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Houses(s)',
			data: [<?= json_encode($total_houses_active); ?>, <?= json_encode($total_houses_disabled); ?>, <?= json_encode($total_houses_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="gradient_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Houses','action' => 'index']]); ?>
				<fieldset>
					<div class="mb-1"><?php echo $this->Form->control('id',['required' => false]); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('owner', ['required' => false]); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('housing_area', ['required' => false]); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('availability', ['required' => false]); ?></div>
					<div class="mb-1"><?php 
						echo $this->Form->label('Houses Status');
						echo '<br/>';
						$options = [
						'0' => 'Blacklist',
						'1' => 'Unverified',
